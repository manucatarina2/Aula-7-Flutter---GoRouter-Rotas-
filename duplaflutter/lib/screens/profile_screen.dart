...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _loadUserInfo() {
    ...
    if (user != null) {
      setState(() {
        ...
        // SOLUÇÃO SIMPLES: pegar a data como string direta
        final createdAtRaw = user.createdAt;
        if (createdAtRaw != null) {
          // Converte para string e pega só a parte da data
          final dateStr = createdAtRaw.toString();
          // Formato típico: "2024-01-15 10:30:00.000Z" ou "2024-01-15"
          if (dateStr.contains(' ')) {
            _userCreatedAt = dateStr.split(' ')[0]; // pega só YYYY-MM-DD
          } else if (dateStr.contains('T')) {
            _userCreatedAt = dateStr.split('T')[0]; // pega só YYYY-MM-DD
          } else {
  ...
  Future<int> _getCount(String type) async {
    ...
    final String tableName = type == 'favoritos' ? 'favorites' : 'history';

    try {
      // SOLUÇÃO SIMPLES: busca e conta no cliente
      final response = await Supabase.instance.client
          .from(tableName)
          .select('id')
          .eq('user_id', user.id);

      // Verifica se response é uma lista e retorna o tamanho
      if (response is List) {
